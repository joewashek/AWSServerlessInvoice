...
WORKDIR /src
COPY ["AWSServerlessInvoice.csproj", "AWSServerlessInvoice/"]
RUN dotnet restore "AWSServerlessInvoice/AWSServerlessInvoice.csproj"

WORKDIR "/src/AWSServerlessInvoice"
COPY . .
RUN dotnet build "AWSServerlessInvoice.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AWSServerlessInvoice.csproj" \
...
Execute unit tests
```
    cd "AWSServerlessInvoice/test/AWSServerlessInvoice.Tests"
    dotnet test
```

Deploy application
```
    cd "AWSServerlessInvoice/src/AWSServerlessInvoice"
